...
**How to apply:** Keep the fade phases explicit (`fadeOut`, `fadeIn`) and track pre-mounted carousel content separately; do not use exit-presence delays for menu-only HUD elements that must be absent from the carousel. The blackout must be explicitly `position: fixed` and rendered above the shell's stacking context, because a child z-index cannot outrank the shared canvas when its parent sits below the canvas. If the blackout is portaled, drive its motion element directly rather than making the portal the direct child of `AnimatePresence`, so animation-completion callbacks advance the handoff reliably.