...
interface MagicOrbState {
  ...
  updateShieldFormTimer: (delta: number) => void;
...
export const useMagicOrb = create<MagicOrbState>()(
  subscribeWithSelector((set, get) => ({
    ...
    activateShield: () => set({ hasShield: true, shieldFormTimer: 0.5 }),
    ...
    updateShieldFormTimer: (delta) => {
      const { shieldFormTimer } = get();
      if (shieldFormTimer <= 0) return;
      set({ shieldFormTimer: Math.max(0, shieldFormTimer - delta) });
    },
    ...
    freezeAllOrbs: () => set((state) => {
      ...
      return {
        darkOrbs: state.darkOrbs.map((orb) => {
          ...
          const dy = orb.position[1] - playerPosition[1];
          return { ...orb, frozen: dx * dx + dy * dy <= r2 };
        }),
      };
    }),
    ...
    spawnSurvivalBoss: () => {
      ...
      const bossHPs: Record<BossType, number> = {
        circle: 100, star: 100, triangle: 100, trapezoid: 100, cube: 100, cloud: 100, arrow: 100, tentacle: 100, monster: 100, bird: 100,
      };
